...
{
   memcpy(&data, &m_globalData, sizeof(CS123SceneGlobalData));
   return true;
}

bool CS123XmlSceneParser :: getCameraData(CS123SceneCameraData& data) const
{
   memcpy(&data, &m_cameraData, sizeof(CS123SceneCameraData));
   return true;
}
...
bool CS123XmlSceneParser :: getLightData(int i, CS123SceneLightData& data) const
{
   ...
   memcpy(&data, m_lights[i], sizeof(CS123SceneLightData));
   return true;
}
...
bool CS123XmlSceneParser::parse() 
{
   ...
   if (!txn->ValueStr().compare("scenefile"))
   {
      ...
      /* Default Camera */
      m_cameraData.pos = Point3(5, 5, 5, 1);
...
bool CS123XmlSceneParser::parseCameraData(const TiXmlElement* scenefile)
{
   ...
   while(cam_element)
   {
      ...
      if (!elem.compare("pos"))
      {
         ...
         m_cameraData.pos = Point3(x, y, z, 1);
      }
